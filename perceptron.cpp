...
public:
  ...
  double get(string str){
	...
	for(int i=0;i<M;i++){
	  ...
	  Min[i] += wlm[L][i] * -1;
	  if(is_debug) printf("Min[%d] = %lf\n", i, Min[i]);
	}

	for(int i=0;i<M;i++){
	  Mout[i] = (double)1 / (1 + exp(-1 * Min[i]));
	  if(is_debug) printf("Mout[%d] = %lf\n",i , Mout[i]);
	}
	...
	for(int i=0;i<N;i++){
	  ...
	  for(int j=0;j<M;j++){
		...
		if(is_debug) printf("Nin[%d](%lf) += wmn[%d][%d](%lf) * Mout[%d](%lf)\n", i, Nin[i], j,i,wmn[j][i], i,Mout[j]);
	  }
	  Nin[i] += wmn[M][i] * -1;
